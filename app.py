...
st.sidebar.subheader("Fotos do Talhão")
fotos = st.sidebar.file_uploader("Envie até 2 fotos do talhão", type=["jpg", "png", "jpeg"], accept_multiple_files=True)
...
if st.sidebar.button("Gerar Análise"):
    with st.spinner("Analisando dados..."):
        ...
        figs = plotar_graficos(dados_pontos, populacao_prevista)
        ...
        if st.button("📥 Baixar Relatório PDF Completo"):
            gerar_relatorio_pdf(fazenda, talhao, cidade, data_avaliacao, dados_pontos, populacao_prevista, recomendacoes, fotos, figs)



# --- Forçar visualização mesmo que dados anteriores não estejam disponíveis ---
st.sidebar.markdown("### 📸 Enviar Fotos do Talhão")
fotos = st.sidebar.file_uploader("Envie até 2 fotos do talhão", type=["jpg", "jpeg", "png"], accept_multiple_files=True)

st.markdown("### 📥 Gerar Relatório Final")
if st.button("📥 Baixar Relatório PDF Completo"):
    try:
        gerar_relatorio_pdf(nome_fazenda, talhao, localizacao, dados_pontos, populacao_prevista, recomendacoes, fotos)
